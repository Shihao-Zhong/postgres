...
collectMatchesForHeapRow(IndexScanDesc scan, pendingPosition *pos)
{
	...
	/*
	 * Reset all entryRes and hasMatchKey flags.  If consistent functions were
	 * called for the previous row, we don't know which entries they might
	 * have changed, so clear everything.
	 */
	for (uint32 i = 0; i < so->nkeys; i++)
	{
		GinScanKey	key = so->keys + i;
 ...
 * false, and remembers nothing, if that would use more than work_mem.
 */
static bool
