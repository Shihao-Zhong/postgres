...
pgstat_fetch_stat_backend(ProcNumber procNumber)
{
	...
	int			pid = GetPGProcByNumber(procNumber)->pid;
...
pgstat_fetch_stat_backend_by_pid(int pid, BackendType *bktype, Oid *userid)
{
	 ...
	 */
	backend_stats = (PgStat_Backend *)
		pgstat_fetch_entry(PGSTAT_KIND_BACKEND, InvalidOid,
						   PGSTAT_BACKEND_OBJID(pid, procNumber), NULL);
	if (!backend_stats)
	{
		if (bktype)
			*bktype = B_INVALID;
